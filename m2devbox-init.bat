...
(
	...
	docker exec -it sand_market_box_web chown -R magento2:magento2 /var/www/sample-data
	RMDIR sample-data /S /Q
	TIMEOUT 5

	ECHO Create symlinks For Sample Data modules, set valid permissions and upgrading database
	docker-compose exec web php -f /var/www/sample-data/dev/tools/build-sample-data.php -- --ce-source="/var/www/magento2"
	docker-compose exec web chown -R :magento2 /var/www/sample-data
	docker-compose exec --user magento2 web sh -c 'find /var/www/sample-data -type d -exec chmod g+ws {} \;'
	docker-compose exec --user magento2 web rm -rf /var/www/sample-data/cache/* /var/www/sample-data/page_cache/* /var/www/sample-data/generation/*
	...
	docker-compose exec --user magento2 web php /var/www/magento2/bin/magento indexer:reindex
